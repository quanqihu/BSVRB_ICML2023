...
class BaseRunner(object):
    ...
    def __init__(self, args):
        ...
        self.gamma_m = args.gamma_m    # for F_NDCG
    ...
    def fit(self, data: BaseModel.Dataset, epoch=-1) -> float:
        ...
        # build an old model, including parameters, moving average estimators, and optimizer states
        if self.loss_type in ['F_NDCG','BSVRB'] and epoch == 1:
            data.model_last_round = copy.deepcopy(model).to(model.device)
            data.model_last_round.train()
        ...
        for batch in tqdm(dl, leave=False, desc='Epoch {:<3}'.format(epoch), ncols=100, mininterval=1):
            if self.loss_type not in ['F_NDCG', 'BSVRB']:
            ...
            elif self.loss_type in ['F_NDCG', 'BSVRB'] and epoch == 1:
                batch = utils.batch_to_gpu(batch, model.device)
                ...
                _, _, _, loss, _ = model.loss(out_dict, epoch)
                loss.backward()
                ...
                model.optimizer.zero_grad()
                data.model_last_round.optimizer.zero_grad()

                batch = utils.batch_to_gpu(batch, model.device)

                out_dict_last_round = data.model_last_round(batch)
                out_dict_last_round = self._add_ids(batch, out_dict_last_round)

                out_dict = model(batch)
                out_dict = self._add_ids(batch, out_dict)

                last_g, last_L_lambda_hessian, last_grad_lambda_q, loss_last_round, _ = data.model_last_round.loss(out_dict_last_round, epoch, None, None, None, compute_last=True)

                #print("loss_last_round:", loss_last_round)

                _, _, _, loss, ma_dict = model.loss(out_dict, epoch, last_g, last_L_lambda_hessian, last_grad_lambda_q)

                #print("loss:", loss)

                grad_last_round = torch.autograd.grad(loss_last_round, data.model_last_round.parameters())

                #data.model_last_round = copy.deepcopy(model) # directly deepcopy is too time-consuming
                data.model_last_round.load_state_dict(model.state_dict())

                if self.loss_type == 'F_NDCG':
                    data.model_last_round.F_NDCG_loss.update_ma_estimators(ma_dict)
                else:
                    data.model_last_round.BSVRB_loss.update_ma_estimators(ma_dict)

                grad = torch.autograd.grad(loss, model.parameters())

                #print("grad_last_round:", grad_last_round)
                #print("grad:", grad)

                for param, g, g_last_round in zip(model.parameters(), grad, grad_last_round):
                    param.grad = g - self.gamma_m * g_last_round
