...
class GeneralModel(BaseModel):
    ...
    def parse_model_args(parser):
        ...
        parser.add_argument('--loss_type', type=str, default='BPR',
                            choices=['RankNet', 'ListNet', 'ListMLE',
                                     ...
                                     'Listwise_CE', 'NDCG', 'F_NDCG', 'BSVRB'],          # ours
        ...
        parser.add_argument('--gamma_u', type=float, default=0.3, help='for F_NDCG')
        parser.add_argument('--gamma_v', type=float, default=0.3, help='for BSVRB')
        parser.add_argument('--gamma_s', type=float, default=0.9, help='for F_NDCG/BSVRB')
        parser.add_argument('--gamma_z', type=float, default=0.9, help='for F_NDCG/BSVRB')
        parser.add_argument('--beta_u', type=float, default=0.1, help='for F_NDCG')
        parser.add_argument('--beta_v', type=float, default=0.1, help='for BSVRB')
        parser.add_argument('--beta_s', type=float, default=0.1, help='for F_NDCG/BSVRB')
        parser.add_argument('--beta_z', type=float, default=0.1, help='for F_NDCG/BSVRB')
        parser.add_argument('--eta1', type=float, default=0.9, help='for BSVRB')
    ...
    def __init__(self, args, corpus):
        ...
        self.gamma_u, self.gamma_v, self.gamma_s, self.gamma_z = args.gamma_u, args.gamma_v, args.gamma_s, args.gamma_z
        self.beta_u, self.beta_v, self.beta_s, self.beta_z = args.beta_u, args.beta_v, args.beta_s, args.beta_z
        self.eta1 = args.eta1
    ...
    def _build_loss_instance(self):
        ...
        elif self.loss_type == 'F_NDCG':
            self.F_NDCG_loss = ndcg_loss.Faster_NDCG_Loss(self.user_num, self.item_num, self.num_pos, 
                                                          self.gamma_u, self.beta_u, self.gamma_s, self.beta_s, self.gamma_z, self.beta_z, k=self.ndcg_topk)
        elif self.loss_type == 'BSVRB':
            self.BSVRB_loss = ndcg_loss.BSVRB_NDCG_Loss(self.user_num, self.item_num, self.num_pos, self.gamma_v, self.beta_v, self.gamma_s, self.beta_s, 
                                                        self.gamma_z, self.beta_z, k=self.ndcg_topk, eta1=self.eta1)
    ...
    def loss(self, out_dict: dict, epoch: int, last_g=None, last_L_lambda_hessian=None, last_grad_lambda_q=None, compute_last=False) -> torch.Tensor:
        ...
        if self.loss_type == 'RankNet':
            ...
            loss = self.F_NDCG_loss(predictions, out_dict, last_g, last_L_lambda_hessian, last_grad_lambda_q, compute_last)
        elif self.loss_type == 'BSVRB':
            loss = self.BSVRB_loss(predictions, out_dict, last_g, last_L_lambda_hessian, last_grad_lambda_q, compute_last)
